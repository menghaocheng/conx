...
if not exist "%PACKAGE_DIR%\install.sh" (
...
python "%SSH_TOOL%" --host %HOST% --user %SSH_USER% --password %SSH_PASSWORD% --root-password %ROOT_PASSWORD% --command "cd %REMOTE_DIR%; chmod +x install.sh uninstall.sh; ./install.sh; /etc/init.d/adbd-tcp status 2>/dev/null || true; ps -ef | grep '[a]dbd' || true"
if errorlevel 1 (
    echo Remote install failed.
    exit /b 1
)

echo Verifying ADB TCP on %HOST%:%ADB_PORT% ...
...
adb -s %HOST%:%ADB_PORT% shell id
if errorlevel 1 (
    echo adb shell verification failed.
    exit /b 1
)
